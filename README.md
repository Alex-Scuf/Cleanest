<h1 align=center>Cleanest <br>
<img width="150" height="150" alt="Без имени-1" src="https://github.com/user-attachments/assets/b7a3b5aa-6a9a-4147-8e20-26cc21685d8d" />
</h1>
<P align=center>A Spicetify theme based on the <a href="https://github.com/Astromations/Hazy">"Hazy"</a> theme
  <BR>An ultra-clean theme</BR></P>

<p align="center">
  <a href="https://github.com/Alex-Scuf/Cleanest">
    <img src="https://img.shields.io/badge/Live%20Preview-GitHub%20Pages-blue?style=for-the-badge&logo=github" alt="Live Preview">
  </a>
</p>

<div align=center>
  <b>⚠️ Good news: I’ve found a working method to get the animated lighting running. IMPORTANT NOTE: This requires a separate Python script which, unfortunately, currently only works on Windows. Follow the steps below to get it working:
    <br>
    <br>
1. Download the "cleanest_audio_bridge.py" file and save it to a location of your choice.
    <br>
2. Open PowerShell and run the command: `pip install PyAudioWPatch websockets numpy`.
     <br>
3. Launch Spotify, go to the theme settings tab, select "Advanced theme settings," and toggle on "Use local audio bridge when unavailable"
    <br>
4. Run `cleanest_audio_bridge.py` (The script must remain open; otherwise, the animation will not work).
<br>
    <br>
That’s it. There is one downside to this method: it captures audio from all windows, not just the Spotify window. For now, however, this is the only working solution to the problem. ⚠️</b></div>
<div>

<details>
<summary>Previews</summary>
<img width="1921" height="1037" alt="preview5" src="https://github.com/user-attachments/assets/422d385a-c439-42d9-b221-eae7f96a7434" />

<img width="1913" height="1037" alt="preview" src="https://github.com/user-attachments/assets/da8fed12-4deb-4d28-8e55-267e7630308c" />

<img width="1910" height="1039" alt="preview7" src="https://github.com/user-attachments/assets/a637ea6f-1f28-423f-957b-19fe1ca6cb87" />

<img width="1916" height="1030" alt="preview9" src="https://github.com/user-attachments/assets/7719cefe-21bf-44ab-bef3-12b4d3295132" />

<img width="1914" height="1035" alt="preview6" src="https://github.com/user-attachments/assets/dafea5a2-8102-4deb-a328-f8d157866166" />

<img width="1920" height="1040" alt="preview8" src="https://github.com/user-attachments/assets/94480273-14f0-4bbf-b75d-810c53e103fb" />
</details>
</div>

<div>
 <details>
<summary>Key differences</summary>

<h2 align=center>1. The "Liked Songs" cover art now adapts to the changing theme color</h2>

<P align=center>
<img width="84" height="358" alt="ezgif com-video-to-gif-converter" src="https://github.com/user-attachments/assets/6aa94b6c-3ac4-4b10-9911-be04d0fe7e2a" />
</P>

<h2 align=center>2. A dynamic glow effect has been added for the track cover art</h2>

<p align=center>
<img width="132" height="193" alt="2026-08-0113-05-45-ezgif com-crop" src="https://github.com/user-attachments/assets/00812c9c-5d96-451b-bfe6-945511b4a7fd" />
</p>
<div align=center>
Since the cover art sits outside the sidebar boundaries, the glow can seamlessly extend beyond them.

You can adjust the glow's brightness and blur in Theme Settings. 

<h2 align=center>3. Screen edge glow</h2>

<p align=center>
<img width="800" height="431" alt="ezgif com-crop" src="https://github.com/user-attachments/assets/5eb3b3d0-84b9-489e-809f-f01d09fd8c90" />
</p>

<h2 align=center>4. A large number of elements that I find useless have been hidden</h2>
<div align=center>
  <b>
  These include:
  <br>
  <br>
Listening activity
  <br>
  <br>
Lyrics (Because I using "Spicy Lyrics")
  <br>
  <br>
Merch
  <br>
  <br>
Mini Player
  <br>
  <br>
Fullscreen
  <br>
  <br>
Credits
  <br>
  <br>
What's New
  <br>
  <br>
Queue
  <br>
  <br>
Connect to a device
  <br>
  <br>
About Artist
  <br>
  <br>
Tours
  <br>
  <br>
Switch to video
  <br>
  <br>
A bunch of misplaced shadows under the elements that ruin the whole theme
  <br>
  <br>
The background of certain context menus, to give them a "Liquid Glass" look
  <br>
  <br>
Recommended tracks at the end of the playlist/album
  </b>
</div>
<br>

<h2 align=center>5. Scalable bottom panel</h2>


<img width="798" height="412" alt="ezgif com-crop (1)" src="https://github.com/user-attachments/assets/09f9dcf2-139a-41bd-b2f5-5e24e9b4f1ee" />
<br>

<h2 align=center>6. Animated ambience Canvas Video</h2>

<img width="140" height="241" alt="ezgif com-crop (2)" src="https://github.com/user-attachments/assets/92720c54-93c9-49de-9473-9646ae93c3cb" />
</details>
</div>
<div>
<details>
<summary>Settings & Compabilities</summary>

<h3 align=center>Standard Settings</h3>
I tried to make the theme as customizable as possible. If you have any suggestions regarding settings, feel free to post them in the <a href="https://discord.gg/CkzuJBFdFf">Spicetify Discord channel</a> under the "themes" tab (that's where I usually hang out).
<br>
<br>
<br>
<b>Here are the standard theme settings:</b>
<br>
 <br>
<img width="931" height="810" alt="Снимок экрана 2026-08-24 141129" src="https://github.com/user-attachments/assets/e82268e2-5391-40c0-8da2-465b7bd245d5" />
<br>
<br>
<b>Here you can:
<br>
<br>
Edit background settings (set a custom or dynamic background, and adjust its color parameters)
<br>
<br>
Change the theme's accent color or use a dynamic one (which adapts based on the track cover art's accent color)
<br>
<br>
Configure static and dynamic cover art glow effects (brightness, size, blur, etc.)
<br>
<br>
Configure static and dynamic screen edge lighting
<br>
<br>
Toggle the visibility of specific elements (About Artist, merch, Queue button, etc.)
<br>
<br>
Enable/disable animated ambience on canvas video
</b>

<h3 align=center>Advanced Settings</h3>
There aren't many options here yet, but I’ve only just started working on this, so please don't judge too harshly)
<br>
<br>
<br>
<b>Here are all the settings:</b>
<br>
<br>
<img width="638" height="740" alt="555" src="https://github.com/user-attachments/assets/a03ebe44-0584-458a-9ab8-42aa35ff029b" />
<br>
<br>
<b>
Here you can:
<br>
<br>
Edit the background color and transparency of the panels (side, central, top, and bottom)
<br>
<br>
Enable compatibility settings for certain extensions and applications that I use (feel free to suggest which extension I should add to the settings next)
</b>
</details>
</div>

<div>
 <details>

<summary>How install</summary>


<h3>Automatically</h3>
<b>
Just search "Cleanest" in Spicetify Marketplace and click on a download-button


<h3>Manually</h3>

  1.Download "color.ini", "user.css" and "theme.js"
  <br>
  2.Create a folder "Cleanest" on the way <em>"C:\Users\(your-username)\AppData\Roaming\spicetify\Themes\"</em> and drop all the downloaded files in there
  <br>
  3.Go to the Powershell and enter <em>"spicetify config current_theme Cleanest"</em>
  <br>
  4.Enter <em>"spicetify apply"</em>
  <br>
</b>
 </details>
</div>
